...
function resolveCssVarColor(input, contextEl) {
  ...
  const varName = raw.slice(4, closeIdx).trim();
  if (!varName.startsWith('--')) return raw;
...
function parseColorToRgbUnit(color, contextEl) {
  const input = resolveCssVarColor(color, contextEl);

  if (typeof input !== 'string' || input.length === 0) return [1, 1, 1];

  if (input.startsWith('#')) {
    let c = input.slice(1).trim();
    if (c.length === 3)
      c = c
        .split('')
        .map((ch) => ch + ch)
  ...
  const rgbMatch = input
    .replace(/\s+/g, '')
    .match(
      /^rgba?\((\d+(?:\.\d+)?),(\d+(?:\.\d+)?),(\d+(?:\.\d+)?)(?:,([01](?:\.\d+)?))?\)$/i
    );
  if (rgbMatch) {
    const r = Math.min(255, Math.max(0, Number(rgbMatch[1]))) / 255;
    const g = Math.min(255, Math.max(0, Number(rgbMatch[2]))) / 255;
    const b = Math.min(255, Math.max(0, Number(rgbMatch[3]))) / 255;
    return [r, g, b];
  }
...
const MetaBalls = ({
  ...
  useEffect(() => {
    ...
    const [r1, g1, b1] = parseColorToRgbUnit(color, container);
    const [r2, g2, b2] = parseColorToRgbUnit(cursorBallColor, container);
